...
prepare_AVARDA_candidate_table <- function(
  ...
  # output_data <- data.frame(matrix(nrow = nrow(AVARDA_subset), ncol = (ncol(AVARDA_subset)-1)))
  ...
  output_data$`Seropos P Value` %<>% formatC(format = "e", digits = 1)
...
prepare_AVARDA_candidate_table_html <- function(data){
  ...
  names(data)[c(2:5, 7:10)] <- rep(c("Case", "Ctrl"), 4)
  options(knitr.table.format = "html")


  # frequency
  data[,c(2:3, 7)] <- data[,c(2:3, 7)] %>% dplyr::mutate_if(is.numeric, function(x){
  ...
  # hit score
  data[,9:10] <- data[,9:10] %>% dplyr::mutate_if(is.numeric, function(x){
    kableExtra::cell_spec(
      x, bold = TRUE,
      color = ifelse(x > 50, "#B4DE2CFF",
                   kableExtra::spec_color(x, end=0.9,
                                          scale_from = c(min(data[,9:10]),50))),
